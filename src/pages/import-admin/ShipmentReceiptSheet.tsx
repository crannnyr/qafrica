...
function Copy({data,label}:{data:ShipmentReceiptData;label:string}) {
  return <div className="relative w-full max-w-[148mm] px-6 py-5 overflow-hidden print:max-w-[148mm] print:px-4 print:py-4">
    ...
    <div className="relative">
      ...
      {data.delivery_address&&<div className="bg-white/70 rounded-lg p-2 mb-3 text-[10px] border border-gray-100"><p className="text-[8px] font-bold text-gray-500 uppercase tracking-wide mb-1">Customer delivery address</p>{typeof data.delivery_address.name==='string'&&data.delivery_address.name.trim()&&<p className="font-semibold text-gray-800">{data.delivery_address.name}{typeof data.delivery_address.phone==='string'&&data.delivery_address.phone.trim()?' · '+data.delivery_address.phone:''}</p>}{typeof data.delivery_address.address_line1==='string'&&data.delivery_address.address_line1.trim()&&<p className="text-gray-600">{data.delivery_address.address_line1}{typeof data.delivery_address.address_line2==='string'&&data.delivery_address.address_line2.trim()?', '+data.delivery_address.address_line2:''}</p>}{((typeof data.delivery_address.city==='string'&&data.delivery_address.city.trim())||(typeof data.delivery_address.state==='string'&&data.delivery_address.state.trim()))?<p className="text-gray-600">{[data.delivery_address.city,data.delivery_address.state].filter(x=>typeof x==='string'&&x.trim()).join(', ')}</p>:typeof data.delivery_address.address==='string'&&data.delivery_address.address.trim()?<p className="text-gray-600">{data.delivery_address.address}</p>:null}</div>}
