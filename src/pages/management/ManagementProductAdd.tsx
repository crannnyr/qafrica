...
export default function ManagementProductAdd() {
  const navigate=useNavigate(); const [f,setF]=useState(initial); const [variants,setVariants]=useState<VariantGroup[]>([]);
  ...
  const save=async()=>{ if(!f.name.trim()||!f.image_url.trim()) return setError('Product name and primary image URL are required.');
    ...
    try{const token=getManagementToken(); if(!token) throw new Error('Management session expired');
      const body:any={manager_token:token,...f,image_urls:f.image_urls.filter(Boolean),variants,has_variants:variants.length>0,
  ...
  return <div className="max-w-5xl space-y-4">
    <div className="flex items-center gap-3"><button onClick={()=>navigate('/management/products')} className="p-2 rounded-lg hover:bg-gray-100"><ArrowLeft className="w-5 h-5"/></button><div><h1 className="font-bold text-gray-900">Add product</h1><p className="text-xs text-gray-400">Same product fields used by Import Admin.</p></div></div>
    ...
    <div className="flex justify-end gap-2 pb-8"><button onClick={()=>navigate('/management/products')} className="px-4 py-2.5 rounded-xl border border-gray-200 text-xs font-semibold">Cancel</button><button onClick={()=>void save()} disabled={saving} className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-orange-500 text-white text-xs font-bold disabled:opacity-60">{saving&&<Loader className="w-4 h-4 animate-spin"/>}Create product</button></div>
  </div>;
}