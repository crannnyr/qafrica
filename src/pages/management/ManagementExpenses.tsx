...
export default function ManagementExpenses(){
 ...
 return <div className="max-w-5xl mx-auto space-y-5">
  <div><h2 className="text-xl font-bold text-gray-900">Expenses & Reimbursements</h2><p className="text-sm text-gray-500 mt-1">Submit money you personally spent for QAfrica so it can be reviewed and paid.</p></div>
  <form onSubmit={submit} className="bg-white rounded-2xl border border-gray-100 p-5 space-y-4">
   ...
   <div className="grid sm:grid-cols-2 gap-4">
    ...
    <div><span className="block text-xs font-semibold text-gray-500 mb-1.5">Receipt</span><label className="cursor-pointer aspect-square max-w-[180px] rounded-xl border-2 border-dashed border-gray-200 flex flex-col items-center justify-center hover:border-gray-400 transition-colors"><Upload className="w-5 h-5 text-gray-300 mb-2"/><span className="text-xs text-gray-400">{file ? 'Replace receipt' : 'Add receipt'}</span><input ref={inputRef} type="file" accept="image/jpeg,image/png,image/webp,application/pdf" className="hidden" onChange={e=>setFile(e.target.files?.[0]||null)}/></label><p className="text-[11px] text-gray-400 mt-1.5">JPG, PNG, WEBP or PDF</p></div>
   </div>
  ...
  <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden"><div className="px-5 py-4 border-b border-gray-100"><p className="font-bold text-gray-900">My submissions</p></div>
