...
export function Step5Shipping({ formData, set, isTerminalMode }: {
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 p-5 shadow-sm space-y-5">
      <h2 className="text-base font-semibold text-gray-900 dark:text-white">Shipping & Inventory</h2>

      {isTerminalMode && (
        <div className="p-3 bg-indigo-50 dark:bg-indigo-500/10 border border-indigo-200 dark:border-indigo-800 rounded-lg flex items-start gap-2">
      ...
      <div className="border-t border-gray-200 dark:border-gray-700 pt-4">
        <h3 className="text-xs font-semibold text-gray-600 dark:text-gray-300 mb-3">Shipping Details</h3>
        <div className="grid sm:grid-cols-2 gap-4">
          <div>
            <label className={labelClass}>
              Weight (kg)
            ...
            <input type="number" step="0.01" min="0.01" value={formData.weight_kg}
              ...
              className={`${inputClass} ${isTerminalMode && !formData.weight_kg ? 'border-orange-300 dark:border-orange-700 bg-orange-50 dark:bg-orange-500/10' : ''}`}
              placeholder="e.g. 0.8" />
          </div>

          <div>
            <label className={labelClass}>
              HS Code <span className="text-gray-400 dark:text-gray-500 font-normal">— Optional</span>
        ...
        <div className="mt-4">
          <label className={labelClass}>
            Dimensions (cm) <span className="text-gray-400 dark:text-gray-500 font-normal">— Optional</span>
          ...
          <p className="text-[10px] text-gray-400 dark:text-gray-500 mt-1">Length × Width × Height in cm</p>
        </div>
      </div>
    </div>
  );
