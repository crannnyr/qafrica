...
import { 
  ...
  ChevronLeft, User, Bell, FileText, AlertTriangle
} from 'lucide-react';
...
const sidebarItems = [
  ...
  { icon: AlertTriangle, label: 'System Failures', path: '/admin/failures' },
        ...
        <nav className="flex-1 p-4 space-y-1 overflow-y-auto custom-scrollbar min-h-0">
          {sidebarItems.map((item) => (
            ...
            >
              ...
              {/* Red dot for domain requests */}
              {item.path === '/admin/domain-requests' && isSidebarOpen && (
                <span className="ml-auto w-2 h-2 bg-red-500 rounded-full"></span>
              )}
              {/* Red dot for system failures */}
              {item.path === '/admin/failures' && isSidebarOpen && (
                <span className="ml-auto w-2 h-2 bg-red-500 rounded-full"></span>
              )}
            </Link>
          ))}
