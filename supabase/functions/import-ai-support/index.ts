...
serve(async(req:Request)=>{
  ...
  const customerScopeBlocked = actor === 'customer' && !isCustomerSupportRequest(message)
  ...
  try{
    ...
    const tools=actor==='admin'?adminTools:actor==='guest'?guestTools:customerTools
    if (customerScopeBlocked) {
      return json({answer:'I can only help with your QAfrica import account and related support information, such as your orders, tracking, payments, bills, refunds, shipping, products, cart, or account issues.',model:MODEL,scope_blocked:true})
    }
