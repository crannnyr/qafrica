...
async function adminSupportAction(s:any, token:string, action:string, body:any) {
  if (action === 'list_support_conversations') {
    ...
    const scope = body.scope === 'ai' ? 'ai' : body.scope === 'resolved' ? 'resolved' : 'human'
    const statuses = scope === 'ai' ? ['ai','returned_to_ai'] : scope === 'resolved' ? ['closed'] : ['human_requested','human_assigned','human_active']
  ...
  if (action === 'take_support_conversation') {
    ...
    if (!['human_requested','human_assigned'].includes(current.status)) {
    ...
    const { data, error } = await s.from('import_ai_whatsapp_conversations')
      ...
      .eq('id', id).in('status', ['human_requested','human_assigned'])
...
const resolveSupportConversationTool = {
  type:'function',
  name:'resolve_support_conversation',
  description:'Mark the authenticated customer support conversation as resolved after the customer has explicitly confirmed that they do not need anything else. Never use this tool immediately after answering a question. First ask whether there is anything else you can help with. Only resolve after the customer clearly says no, that is all, thanks/no more help, or an equivalent confirmation.',
  parameters:{type:'object',properties:{reason:{type:'string',maxLength:300}},required:['reason'],additionalProperties:false}
}
const whatsappCustomerTools = [...customerTools, requestHumanSupportTool, resolveSupportConversationTool]
...
async function customerTool(s:any, req:Request, actor:Actor, name:string, a:any, context:{channel:string,conversationId:string|null}) {
  ...
  if (name === 'resolve_support_conversation') {
    if (actor !== 'customer') throw new Error('Only an authenticated customer can resolve this support conversation')
    const conversationId = clean(context.conversationId, 80)
    if (!conversationId) throw new Error('Support conversation is not available')
    const now = new Date().toISOString()
    const { data, error } = await s.from('import_ai_whatsapp_conversations')
      .update({ status:'closed', updated_at:now })
      .eq('id', conversationId)
      .eq('customer_id', id)
      .in('status', ['ai','returned_to_ai','human_active','human_assigned','human_requested'])
      .select('id,status,channel,updated_at')
      .maybeSingle()
    if (error) throw error
    if (!data) throw new Error('Support conversation could not be resolved')
...
serve(async(req:Request)=>{
  ...
  try{
    ...
    if (actor==='customer' && channel==='website') {
      ...
      if (websiteConversation.status === 'closed') {
        const now = new Date().toISOString()
        const { data: reopened, error: reopenError } = await s.from('import_ai_whatsapp_conversations')
          .update({ status:'ai', updated_at:now })
          .eq('id', websiteConversation.id)
          .eq('customer_id', await customerId(s,req))
          .select('id,status')
          .single()
        if (reopenError) throw reopenError
        websiteConversation = reopened
      }
    ...
    for(let turn=0;turn<5;turn++){
      ...
      if(!calls.length){
        ...
        const resolved = websiteConversation?.status === 'closed'
        return json({answer,model:MODEL,conversation_id:channel==='website'?websiteConversation?.id:null,status:channel==='website'?(resolved?'closed':websiteConversation?.status):null,handed_off:false,resolved})
      }
      ...
      for(const c of calls){
        ...
        let result:any
        try{
          result=actor==='admin'?await adminTool(s,token!,c.name,a):await customerTool(s,req,actor,c.name,a,{channel,conversationId})
          if (c.name === 'resolve_support_conversation' && result?.success && actor === 'customer' && channel === 'website') {
            websiteConversation = result.conversation
          }
        }catch(e){result={error:e instanceof Error?e.message:'Tool execution failed'}}
