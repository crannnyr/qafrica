...
const customerTools = [
  ...
  { type:'function', name:'get_my_order_context', description:'Get the complete customer-safe tracking context for one authenticated order: overall order stage, payment state, shipping method, fulfillment items with ordered/received/allocated/shipped/delivered quantities and item status, every active shipment with its status, carrier, tracking details, shipment items and shipment events, plus the customer-facing consolidation & shipping bill, delivery estimate, refunds and expired-order records. Use this for every specific tracking, delivery, shipment, item-progress, partial-shipment, partial-delivery, arrival-in-Nigeria, or where-is-my-order question.', parameters:{type:'object',properties:{code:{type:'string'}},required:['code'],additionalProperties:false}},
  ...
  { type:'function', name:'mark_conversation_resolved', description:'Mark the current AI-handled support conversation as resolved when the customer confirms the issue is resolved or clearly indicates no further help is needed. Never use this while a human agent is actively handling the conversation.', parameters:{type:'object',properties:{reason:{type:'string',maxLength:300}},required:['reason'],additionalProperties:false}},
]
...
async function customerTool(s:any, req:Request, actor:Actor, name:string, a:any, context:{channel:string,conversationId:string|null}) {
  ...
  if (name === 'mark_conversation_resolved') {
    if (actor !== 'customer') throw new Error('Only a customer conversation can be resolved by the AI')
    if (!context.conversationId) throw new Error('Support conversation context is required')
    const now = new Date().toISOString()
    const { data: conversation, error: ce } = await s.from('import_ai_whatsapp_conversations')
      .select('id,status,customer_id,channel')
      .eq('id', context.conversationId).maybeSingle()
    if (ce) throw ce
    if (!conversation) throw new Error('Support conversation not found')
    if (conversation.customer_id !== id) throw new Error('Support conversation does not belong to this customer')
    if (!['ai','returned_to_ai'].includes(conversation.status)) {
      return { success:true, status:conversation.status, already_closed:conversation.status === 'resolved' }
    }
    const { data, error } = await s.from('import_ai_whatsapp_conversations')
      .update({ status:'resolved', human_agent_id:null, updated_at:now })
  ...
  if (name === 'get_my_order_context') {
    ...
    const [{data:bills,error:be},{data:refunds,error:re},{data:fulfillmentItems,error:fe},{data:shipments,error:se}] = await Promise.all([
      ...
      s.from('china_import_fulfillment_items')
        ...
        .eq('order_id',order.id).order('order_item_index',{ascending:true}),
      s.from('china_import_shipments')
        ...
        .eq('order_id',order.id).neq('status','cancelled').order('created_at',{ascending:true})
    ]);
    if(be) throw be; if(re) throw re; if(fe) throw fe; if(se) throw se;

    const shipmentIds=(shipments??[]).map((x:any)=>x.id);
    const [{data:shipmentItems,error:sie},{data:shipmentEvents,error:see}] = await Promise.all([
      shipmentIds.length
        ? s.from('china_import_shipment_items').select('shipment_id,fulfillment_item_id,quantity').in('shipment_id',shipmentIds)
        : Promise.resolve({data:[],error:null}),
      shipmentIds.length
        ? s.from('china_import_fulfillment_events').select('id,shipment_id,fulfillment_item_id,event_type,status,location,note,created_at').in('shipment_id',shipmentIds).order('created_at',{ascending:true})
        : Promise.resolve({data:[],error:null})
    ]);
    if(sie) throw sie; if(see) throw see;

    const itemById=new Map((fulfillmentItems??[]).map((x:any)=>[x.id,x]));
    const publicShipments=(shipments??[]).map((shipment:any)=>({
      ...shipment,
      items:(shipmentItems??[]).filter((x:any)=>x.shipment_id===shipment.id).map((x:any)=>{
        const item=itemById.get(x.fulfillment_item_id);
        return {
          fulfillment_item_id:x.fulfillment_item_id,
          quantity:x.quantity,
      ...
      events:(shipmentEvents??[]).filter((x:any)=>x.shipment_id===shipment.id).map((x:any)=>({
    ...
    const stageMap:any = {
      ...
      clearance_and_closed:{label:'Received at QAfrica HQ',description:'Your shipment has arrived at QAfrica HQ in Nigeria and is being prepared for final delivery or pickup.'},
      received:{label:'Received at QAfrica HQ',description:'Your shipment has arrived at QAfrica HQ in Nigeria and is being prepared for final delivery or pickup.'},
      delivered:{label:'Delivered',description:'The order has been delivered successfully.'}
    };
