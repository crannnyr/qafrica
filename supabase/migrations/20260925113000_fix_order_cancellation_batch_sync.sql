...
returns table (
  customer_id uuid, customer_name text, first_order_at timestamptz, order_count int,
  shipping_method text, order_id uuid, order_code text, product_id uuid,
  product_name text, product_image text, source_url text, ship_only boolean,
  variant_options jsonb, qty int, unit_price_ngn numeric,
  others_in_batch int, customers_in_open_batch int
)
language sql stable set search_path to 'public', 'pg_temp'
as $function$
  with batch_items as (
    select o.user_id customer_id, o.customer_name, o.shipping_method, o.id order_id,
      o.code order_code, o.created_at order_created_at,
      (item->>'id')::uuid product_id, item->>'name' product_name,
      nullif(item->>'image_url','') product_image,
      coalesce((item->>'quantity')::int,1) qty,
      coalesce((item->>'price_ngn')::numeric,0) unit_price_ngn,
  ...
  select bi.customer_id,bi.customer_name,ct.first_order_at,ct.order_count::int,
    bi.shipping_method,bi.order_id,bi.order_code,bi.product_id,bi.product_name,
    coalesce(bi.product_image,p.image_url),p.source_url,coalesce(p.ship_only,false),
    bi.variant_options,bi.qty,bi.unit_price_ngn,
    greatest(coalesce(bpc.customers_in_batch,1)-1,0)::int,
    coalesce(obp.customers_in_open_batch,0)::int
  from batch_items bi
...
begin
  ...
  if v_bill_id is not null and not v_bill_paid then
    update public.china_import_consolidation_bills set status='cancelled',updated_at=now()
  ...
  return jsonb_build_object('success',true,'reconciled',true,
    'batch_key',v_batch_key,'customer_id',v_customer_id,
    'bill_reset',(v_bill_status='sent' and not v_bill_paid));
end;
$function$;
