-- Correct the live dashboard queues to use the same sources as the
-- Import Admin Refunds, Fulfillment and AI Support screens.
...
as $function$
  ...
  live_ops as (
    select jsonb_build_object(
      ...
      'units_awaiting_arrival', (select coalesce(sum(greatest(ordered_quantity - received_quantity, 0)), 0) from china_import_fulfillment_items where status = 'awaiting_arrival'),
      'fulfillment_open_lines', (select count(*) from china_import_fulfillment_items where status = 'awaiting_arrival'),
      'fulfillment_hq_units', (select coalesce(sum(received_quantity), 0) from china_import_fulfillment_items where status = 'at_qafrica_hq'),
      'fulfillment_allocated_units', (select coalesce(sum(allocated_quantity), 0) from china_import_fulfillment_items where status = 'fully_allocated'),
      ...
      'pending_refunds_count', (select count(*) from china_import_refunds where status in ('pending', 'submitted')),
      'pending_refunds_value_ngn', (select coalesce(sum(refund_amount_ngn), 0) from china_import_refunds where status in ('pending', 'submitted')),
      'active_batches_count', (select count(*) from import_batches where opened_at is not null and clearance_closed_at is null),
      'support_ai_count', (select count(*) from import_ai_whatsapp_conversations where status in ('ai', 'returned_to_ai')),
      'support_waiting_count', (select count(*) from import_ai_whatsapp_conversations where status = 'human_requested'),
      'support_human_active_count', (select count(*) from import_ai_whatsapp_conversations where status in ('human_assigned', 'human_active')),
      'support_resolved_count', (select count(*) from import_ai_whatsapp_conversations where status = 'resolved')
    ) as snapshot
  )
  select jsonb_build_object(
