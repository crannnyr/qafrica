-- Step 5E: prevent checkout-prepaid shipping from being billed again at consolidation.
--
-- Product-price reconciliation is unchanged.
-- For consolidation_shipping, shipping is charged only for order lines whose
-- order does not already have prepaid_shipping_ngn > 0.
-- This supports a mixed batch containing legacy (post-consolidation shipping)
-- orders and newer checkout-prepaid orders without changing existing bills.
